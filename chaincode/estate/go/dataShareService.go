...
func (t *SimpleChainCode) saveDataShare(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if dataShare.Status == "0"{
		...
		return shim.Error("Invalid status. Expecting \"0\" ")
	}
	
	// 创建索引
	// 这里根据提案人和受理人分别创建了索引
	indexKey, err := stub.CreateCompositeKey(COMPOSITE_KEY_ASSET_TYPE_PROPOSER_STATUS_TARGET_KEY, []string{dataModel.AssetType,dataModel.Proposer,dataShare.Status,dataShare.TargetKey})
	if err != nil {
		return shim.Error(err.Error())
	}
	// 索引上链
 ...
 * 数据授权
 */
func (t *SimpleChainCode) approval (stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	// 授权状态不能为初始状态
	if dataShare.Status == "0"{
		return shim.Error("Invalid status. Expecting \"1\" \"2\" ")
	}else{
		// 申请受理后 删除原组合键
		indexKey, _ := stub.CreateCompositeKey(COMPOSITE_KEY_ASSET_TYPE_PROPOSER_STATUS_TARGET_KEY, []string{dataModel.AssetType,dataModel.Proposer,"0",dataShare.TargetKey})
